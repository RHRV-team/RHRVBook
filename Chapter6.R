...
hrv.data <- HRVData
hrv.data <- BuildNIHR(hrv.data)
hrv.data <- LoadEpisodesAscii(hrv.data,
  "./sampleData/apnea_ascii.txt")
ListEpisodes(hrv.data)
...
hrv.data <- LoadBeatAscii(hrv.data,
   "./sampleData/a01.beats.txt")
hrv.data <- LoadEpisodesAscii(hrv.data,
   "./sampleData/a01.tags.txt")


## ----chapter6_addepisodes-----------------------------------------------------
hrv.data <- AddEpisodes(hrv.data,
   InitTimes = c(5000, 15000), Durations = c(2000, 2000),
   Tags = c("Tag1", "Tag2"), Values = c(0, 0))
ListEpisodes(hrv.data)
...
hrv.data <- ModifyEpisodes(hrv.data, Tags = "Tag1",
   Indexes = 3, NewDurations = c(300, 400),
   NewValues = 8.5)
ListEpisodes(hrv.data)
...
remove(hrv.data)
hrv.data <- CreateHRVData()
hrv.data <- LoadBeatRR(hrv.data, "./sampleData/rr.txt")
